...
def page_vri(model, scaler, features, model_key):
    """Page dédiée à l'Indice de Risque Vectoriel (VRI)"""
    ...
    with st.form(key="vri_form"):
        ...
        if submitted:
            ...
            if features is not None:
                ...
                st.info("Utilisation de l'ordre par défaut des features.")
            ...
            if pred_clamped < 0.2:
                interpretation = "✅ **Risque très faible** – Conditions défavorables."
            elif pred_clamped < 0.4:
                interpretation = "⚠️ **Risque faible** – Surveillance recommandée."
            elif pred_clamped < 0.6:
                interpretation = "⚠️ **Risque modéré** – Prévention."
            elif pred_clamped < 0.8:
                interpretation = "🔥 **Risque élevé** – Prolifération probable."
            else:
                interpretation = "🚨 **Risque très élevé** – Actions urgentes."
            st.markdown(f"**Interprétation** : {interpretation}")
            st.progress(pred_clamped, text="Niveau de risque")

# ==================== PAGES SIMPLIFIÉES POUR LES AUTRES MODÈLES ====================
# (à compléter selon vos besoins, ici un exemple minimal pour que la navigation fonctionne)

def page_generic(model, scaler, features, model_name, description, input_fields):
    """Page générique pour les autres modèles (à personnaliser)"""
...
# Exemples de configurations pour quelques modèles (vous devez adapter selon vos modèles réels)
PAGES_CONFIG = {
    "Indice de Stress Thermique (HSI)": {
        "key": "hsi",
        "description": "Indice combinant température et humidité pour évaluer la sensation thermique.",
        "fields": {
    ...
    "Indice de Qualité de l'Air (IQA)": {
        "key": "iqa",
        "description": "Proxy de qualité de l'air basé sur stagnation, rayonnement, saison.",
        "fields": {
...
if option == "Risque Vectoriel (VRI)":
    ...
    # Page générique (à améliorer avec les champs exacts pour chaque modèle)
